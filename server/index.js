...
app.post('/api/sync-columns', async (req, res) => {
  try {
    ...
    // ── 3. QAQC_PROJECTS — full rebuild from PROJECTS where QA/QC = Yes ────────
    // Formula: Actual Eff = (ClientHrs/TotalSpent)/10  | Exp Eff = (ClientHrs/ReqEff)/10
    // Uses ONLY derived QC hours (hoursMap["{id}-QC"]) and QA/QC Assigned Person (col 21)
    const qaqcRows = newProjRows.filter((r) => r[20] === 'Yes').map((r) => {
      const qcH   = hoursMap[`${r[0]}-QC`]       || { spent: 0, reqEff: 0, actEff: 0 };
      const clientHrs  = parseFloat(r[7]) || 0;
      const totalSpent = qcH.spent;   // ONLY derived QC hours
      const reqEffHrs  = qcH.reqEff;
      const actEffHrs  = qcH.actEff;
      const actualEff  = totalSpent > 0 ? +(((clientHrs / (totalSpent / 60)) / 10).toFixed(2)) : '';
      const expEff     = reqEffHrs  > 0 ? +(((clientHrs / (reqEffHrs / 60))  / 10).toFixed(2)) : '';
      // Use QA/QC Assigned Person (col 21) as Leading Person, fallback to Team Lead (col 4)
      const leadingPerson = r[21] || r[4] || '';
      return [r[0], r[1], r[3], r[2], clientHrs || '', +(totalSpent).toFixed(2) || '', +(reqEffHrs).toFixed(2) || '', +(actEffHrs).toFixed(2) || '', actualEff, expEff, leadingPerson];
    });
    await clearRange(SHEETS.QAQC, 'A1:K500');
    await setValues(SHEETS.QAQC, 'A1', [QAQC_HEADERS, ...qaqcRows]);
    results.push({ sheet: SHEETS.QAQC, rows: qaqcRows.length, note: 'Rebuilt from PROJECTS (QA/QC = Yes)' });

    // ── 4. FEEDBACK_PROJECTS — full rebuild from PROJECTS where Client Feedback = Yes ─
    // Formula: Actual Eff = (TotalSpent/ClientHrs)*100 | Exp Eff = (ReqEff/ClientHrs)*100
    // Uses ONLY derived FB-Client hours (hoursMap["{id}-FB-Client"]) and FB Client Assigned Person (col 23)
    const fbRows = newProjRows.filter((r) => r[18] === 'Yes').map((r) => {
      const fbCH  = hoursMap[`${r[0]}-FB-Client`]  || { spent: 0, reqEff: 0, actEff: 0 };
      const clientHrs   = parseFloat(r[7]) || 0;
      const totalSpent  = fbCH.spent;   // ONLY derived FB-Client hours
      const reqEffHrs   = fbCH.reqEff;
      const actEffHrs   = fbCH.actEff;
      const clientFbHrs = parseFloat(r[19]) || 0;
      const actualEff   = clientHrs > 0 ? +((totalSpent / 60 / clientHrs).toFixed(2)) : '';
      const expEff      = clientHrs > 0 ? +((reqEffHrs  / 60 / clientHrs).toFixed(2)) : '';
      // Use FB Client Assigned Person (col 23) as Leading Person, fallback to Team Lead (col 4)
      const leadingPerson = r[23] || r[4] || '';
      return [r[0], r[1], r[3], r[2], clientHrs || '', +(totalSpent).toFixed(2) || '', +(reqEffHrs).toFixed(2) || '', +(actEffHrs).toFixed(2) || '', clientFbHrs || '', actualEff, expEff, leadingPerson];
    });
    await clearRange(SHEETS.FEEDBACK, 'A1:L500');
    await setValues(SHEETS.FEEDBACK, 'A1', [FEEDBACK_HEADERS, ...fbRows]);
    results.push({ sheet: SHEETS.FEEDBACK, rows: fbRows.length, note: 'Rebuilt from PROJECTS (Client Feedback = Yes)' });

    // ── 5. INT_FEEDBACK_PROJECTS — rebuild from PROJECTS where Internal Feedback = Yes ──
    // Formula: same as QAQC: Actual = (ClientHrs/TotalSpent)/10 | Exp = (ClientHrs/ReqEff)/10
    // Uses ONLY derived FB-Int hours (hoursMap["{id}-FB-Int"]) and FB Internal Assigned Person (col 22)
    const intFbRows = newProjRows.filter((r) => r[15] === 'Yes').map((r) => {
      const fbIH  = hoursMap[`${r[0]}-FB-Int`]     || { spent: 0, reqEff: 0, actEff: 0 };
      const clientHrs  = parseFloat(r[7]) || 0;
      const totalSpent = fbIH.spent;   // ONLY derived FB-Int hours
      const reqEffHrs  = fbIH.reqEff;
      const actEffHrs  = fbIH.actEff;
      const actualEff  = totalSpent > 0 ? +(((clientHrs / (totalSpent / 60)) / 10).toFixed(2)) : '';
      const expEff     = reqEffHrs  > 0 ? +(((clientHrs / (reqEffHrs / 60))  / 10).toFixed(2)) : '';
      // Use FB Internal Assigned Person (col 22) as Leading Person, fallback to Team Lead (col 4)
    ...
    results.push({ sheet: SHEETS.INT_FB, rows: intFbRows.length, note: 'Rebuilt from PROJECTS (Internal Feedback = Yes)' });
...
app.delete('/api/attendance/session/:row', async (req, res) => {
  try {
    await clearRange(SHEETS.ATTENDANCE, `A${req.params.row}:N${req.params.row}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/attendance', async (req, res) => {
  try {
    ...
    res.json({ headers: ATT_HEADERS, data: rows.filter((r) => r[0] && r[1]) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function syncEmployeeExperience() {
  ...
  const updates = [];
  rows.forEach((row, i) => {
    const joinDate = row[11] || '';
    if (!joinDate) return;

    const months = calcExpMonths(joinDate);
