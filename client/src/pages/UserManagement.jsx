...
function BackupFileManager({ toast }) {
  ...
  if (loading) {
    return <LoadingScreen message="Loading backups" words={['archives', 'snapshots', 'versions', 'copies', 'archives']} />;
  }
...
export default function UserManagement({ toast, currentUser }) {
  ...
  return (
    <div>
      ...
      {/* User list */}
      {loading ? (
        <LoadingScreen message="Managing users" words={['accounts', 'permissions', 'roles', 'access', 'accounts']} />
      ) : (
        <div className="space-y-2">
          {users.length === 0 && (
          ...
          {users.map((u) => (
            <div key={u.row} className="bg-white border border-slate-100 rounded-xl px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
