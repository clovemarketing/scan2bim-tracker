...
const PROJ_BAR_COLORS = ['#6366f1', '#8b5cf6', '#ec4899', '#06b6d4', '#10b981', '#f59e0b', '#ef4444'];
...
export default function Dashboard({ toast, navigate }) {
  ...
  if (loading || !data) return (
    <div className="page flex items-center justify-center h-full">
      <span className="text-slate-400 animate-pulse">Loading dashboard…</span>
    </div>
  );

  const overallEffPct = data.overallEff ? Math.round(data.overallEff * 100) : 0;
  const projChartData = filteredProjStats.slice(0, 10).map((p) => ({
    ...
    <div className="page">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="page-sub">BIM Project Management · Live from Google Sheets</p>
        </div>
        <button onClick={() => { loadMain(); loadAttSummary(attFilter); }} className="btn-secondary text-xs gap-1.5">
          ...
          <div className="flex gap-1 bg-slate-100 p-0.5 rounded-lg">
            {ATT_FILTERS.map((f) => (
              <button key={f.key} onClick={() => setAttFilter(f.key)}
                className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${attFilter === f.key ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>
                {f.label}
              </button>
            ))}
          </div>
      ...
      {data.projStats?.length > 0 && (
        <div className="card mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('project-progress')} className="text-xs text-indigo-600 hover:underline">View all →</button>
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-end mb-4">
            ...
            <div>
              <select className="input w-48 text-xs" value={projEffFilter} onChange={(e) => setProjEffFilter(e.target.value)}>
      ...
      {data.empEff?.length > 0 && (
        <div className="card mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('project-hours')} className="text-xs text-indigo-600 hover:underline">View full log →</button>
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-end mb-4">
            <div>
              ...
              <select className="input w-44 text-xs" value={effDept} onChange={(e) => setEffDept(e.target.value)}>
            ...
            <div>
              ...
              <select className="input w-48 text-xs" value={effTier} onChange={(e) => setEffTier(e.target.value)}>
                {EFF_TIERS.map((t) => <option key={t.key} value={t.key}>{t.label}</option>)}
              </select>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          {data.activeProjects?.length ? (
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100">
                <th className="text-left py-2 text-xs font-medium text-slate-500">Project</th>
                <th className="text-left py-2 text-xs font-medium text-slate-500">Lead</th>
                <th className="text-left py-2 text-xs font-medium text-slate-500">Client Hrs</th>
              </tr></thead>
              <tbody>
                {data.activeProjects.map((p, i) => (
                  <tr key={i} className="border-b border-slate-50">
                    <td className="py-2 font-medium text-slate-700 max-w-[200px] truncate">{p[1]}</td>
                    <td className="py-2 text-slate-500">{p[4]}</td>
                    <td className="py-2 text-slate-500">{p[7]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('attendance')} className="text-xs text-indigo-600 hover:underline">View all →</button>
          </div>
          {data.recentAtt?.length ? (
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100">
                <th className="text-left py-2 text-xs font-medium text-slate-500">Date</th>
                <th className="text-left py-2 text-xs font-medium text-slate-500">Employee</th>
                <th className="text-left py-2 text-xs font-medium text-slate-500">Project</th>
                <th className="text-left py-2 text-xs font-medium text-slate-500">Hrs</th>
              </tr></thead>
              <tbody>
                {data.recentAtt.map((r, i) => (
                  <tr key={i} className="border-b border-slate-50">
                    <td className="py-2 text-xs text-slate-500">{r[0]}</td>
                    <td className="py-2 font-medium text-slate-700">{r[2]}</td>
                    <td className="py-2 text-slate-500 text-xs max-w-[140px] truncate" title={r[6]}>{r[6] || '—'}</td>
                    <td className="py-2 text-slate-500">{r[9] ? toHhmm(r[9]) : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="card">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <select className="input w-24 text-xs" value={monthFilter} onChange={(e) => { setMonthFilter(e.target.value); setSelectedProj(null); }}>
          ...
          {monthFilter === 'custom' && (
            <>
              <input type="month" className="input w-36 text-xs" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} placeholder="From" />
              <span className="text-xs text-slate-400">to</span>
              <input type="month" className="input w-36 text-xs" value={customTo} onChange={(e) => setCustomTo(e.target.value)} placeholder="To" />
            </>
          )}
          <div className="relative">
            <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" />
            <input className="input pl-6 w-32 text-xs" placeholder="Search..." value={search} onChange={(e) => { setSearch(e.target.value); setSelectedProj(null); }} />
          </div>
          <select className="input w-28 text-xs" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setSelectedProj(null); }}>
