...
import { Plus, Pencil, Trash2, RefreshCw, UserCheck, UserX, Users, Briefcase, Building2, Clock, Sparkles, AlertCircle, CheckCircle2, Search, Upload, Download, X } from 'lucide-react';
...
export default function Employees({ toast }) {
  ...
  const syncExp = async () => {
    setSyncing(true);
    try {
      const r = await api.syncExperience();
      toast.success(`Experience synced — ${r.updated} employee(s) updated`);
      if (r.updated > 0) load();
    } catch (e) { toast.error(e.message); }
    ...
    <div className="page">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={syncExp} disabled={syncing} className="btn-secondary gap-1.5" title="Recalculate experience from join date and update Req Eff Ratio from productivity matrix">
            <Sparkles size={14} className={syncing ? 'animate-pulse' : ''} />
            {syncing ? 'Syncing…' : 'Sync Experience'}
          </button>
          <button onClick={() => setShowIncomplete(true)}
            className={`btn-secondary gap-1.5 relative ${incompleteRows.length > 0 ? 'text-amber-600 border-amber-300 hover:bg-amber-50' : 'text-emerald-600 border-emerald-200 hover:bg-emerald-50'}`}>
            {incompleteRows.length > 0
              ? <AlertCircle size={14} />
              : <CheckCircle2 size={14} />}
            {incompleteRows.length > 0 ? `Incomplete (${incompleteRows.length})` : 'All Complete'}
          </button>
          <button onClick={exportToExcel} className="btn-secondary gap-1.5"><Download size={14} /> Export</button>
          <button onClick={() => setImportOpen(true)} className="btn-secondary gap-1.5"><Upload size={14} /> Import</button>
          <button onClick={openAdd} className="btn-primary"><Plus size={14} /> Add Employee</button>
        </div>
      </div>
      ...
      <div className="card mb-5 flex flex-wrap gap-3 items-end">
        <div>
          ...
          <select className="input w-40" value={fDesignation} onChange={(e) => setFDesignation(e.target.value)}>
        ...
        <div>
          ...
          <select className="input w-40" value={fDept} onChange={(e) => setFDept(e.target.value)}>
            <option value="All">All</option>
        ...
        <div>
          ...
          <select className="input w-44" value={fTeamLead} onChange={(e) => setFTeamLead(e.target.value)}>
            <option value="All">All</option>
        ...
        <div>
          ...
          <select className="input w-36" value={fShift} onChange={(e) => setFShift(e.target.value)}>
            <option value="All">All</option>
        ...
        <div>
          ...
          <select className="input w-36" value={fStatus} onChange={(e) => setFStatus(e.target.value)}>
            <option value="All">All</option>
      ...
      {loading ? <p className="text-slate-400 animate-pulse">Loading…</p> : (
        <DataTable
          ...
          selectable
          defaultHiddenCols={[2, 7, 8, 9, 10, 11, 12]}
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Add Employee' : 'Edit Employee'} onClose={() => setModal(null)} wide>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-3"><Field form={form} set={set} label="Remarks" i={12} /></div>
          </div>
            ...
            <>
               ...
               <div className="flex flex-wrap gap-2 mb-3">
                 <div className="relative flex-1 min-w-36">
                   ...
                   <input className="input pl-8 pr-8 text-sm" placeholder="Search name or EMP ID…"
                ...
                <select className="input w-44 text-sm" value={incompleteFTL} onChange={(e) => setIncompleteFTL(e.target.value)}>
                  <option value="All">All Team Leads</option>
                  <option value="None">— Not assigned —</option>
                  {incompleteTLOpts.map(tl => <option key={tl}>{tl}</option>)}
                </select>
                <select className="input w-40 text-sm" value={incompleteFShift} onChange={(e) => setIncompleteFShift(e.target.value)}>
        ...
        <Modal title="Import Employees from Excel" onClose={() => { setImportOpen(false); setImportStep('upload'); setMapping({}); setExcelHeaders([]); setExcelData([]); setSheetDataMap({}); setSheetNames([]); setSelectedSheet(''); }} wide>
          {importStep === 'upload' ? (
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 mb-4 max-h-52 overflow-y-auto">
