    ...
    <div className="border border-slate-200 rounded-xl p-3">
      ...
      {!expanded && selectedList.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {selectedList.map((e) => (
            <span key={e[0]} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-[10px] font-medium">
              {e[1]}
          ...
          <div className="border border-slate-200 rounded-xl max-h-40 overflow-y-auto">
            {employees.length === 0 ? (
              ...
              <table className="min-w-full text-xs">
                ...
                <tbody>
                  {employees.map((e) => (
                    <tr key={e[0]} className="tr hover:bg-indigo-50/30">
...
export default function Projects({ toast }) {
  ...
  return (
    <div className="page">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-slate-100 rounded-lg p-0.5 mr-1">
            <button onClick={() => setTab('projects')}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${tab === 'projects' ? 'bg-white text-slate-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
              Projects
            </button>
            <button onClick={() => setTab('assignments')}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${tab === 'assignments' ? 'bg-white text-slate-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
              Assignments
            </button>
          </div>
            ...
            <div className="flex gap-2 flex-wrap items-center">
              {['All', ...STATUSES].map((s) => (
                <button key={s} onClick={() => setFilter(s)}
                  className={`px-3 py-1 rounded-full text-xs font-medium border transition-all ${filter === s ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'}`}>
                  {s} {s !== 'All' && <span className="opacity-60">({statusCounts[s] || 0})</span>}
                </button>
              ))}
            </div>
            <div className="ml-auto flex items-center gap-2">
              <label className="text-xs text-slate-500 font-medium whitespace-nowrap">Team Lead</label>
              <select className="input w-48 text-xs" value={filterLead} onChange={(e) => setFilterLead(e.target.value)}>
                <option value="All">All Team Leads</option>
                {teamLeads.map((l) => <option key={l}>{l}</option>)}
              </select>
              {(filter !== 'All' || filterLead !== 'All') && (
                <button className="btn-secondary text-xs" onClick={() => { setFilter('All'); setFilterLead('All'); }}>Clear</button>
              )}
            </div>
          </div>
        ...
        <>
          <div className="card mb-5 flex flex-wrap gap-4 items-end">
            <div>
              ...
              <select className="input w-40" value={mapFilterLead} onChange={(e) => setMapFilterLead(e.target.value)}>
                <option>All</option>
            ...
            <div>
              ...
              <select className="input w-56" value={mapFilterProj} onChange={(e) => setMapFilterProj(e.target.value)}>
                <option>All</option>
            ...
            <div>
              ...
              <select className="input w-32" value={mapFilterStatus} onChange={(e) => setMapFilterStatus(e.target.value)}>
                <option>All</option>
                {MAP_STATUSES.map((s) => <option key={s}>{s}</option>)}
              </select>
            </div>
            <div className="text-sm text-slate-500 self-end pb-2 ml-auto">{mapDisplayed.length} results</div>
          </div>
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Add Project' : 'Edit Project'} onClose={() => setModal(null)} wide>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-3"><label className="label">Remarks</label><input className="input" value={form[11] ?? ''} onChange={(e) => set(11, e.target.value)} /></div>
            ...
            <div className="col-span-3">
              ...
              {form[12] === 'Yes' && form[19] && (
                <div className="col-span-3 border-t border-slate-100 pt-3 mt-3">
                  <EmployeeChecklist
      ...
      {/* ── Import Modal ── */}
      {importOpen && (
        <Modal title="Import Projects from Excel" onClose={() => { setImportOpen(false); setImportStep('upload'); setMapping({}); }} wide>
          {importStep === 'upload' ? (
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mb-3">
                Map your Excel columns to system fields. <span className="text-red-400">* Required</span>
              </p>
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 mb-4 max-h-48 overflow-y-auto">
                {SYSTEM_COLS.map((sysCol, sysIdx) => {
                  const isReq = REQUIRED_COLS.includes(sysIdx);
                  return (
                    <div key={sysIdx} className="flex items-center gap-2">
                      <span className="text-xs text-slate-700 w-36 shrink-0 truncate font-medium">
                        {sysCol} {isReq && <span className="text-red-400">*</span>}
                      </span>
              ...
              <p className="text-xs text-slate-400 mb-2">Preview ({mappedRows.length} rows):</p>
              <div className="max-h-52 overflow-auto border border-slate-200 rounded-lg">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-50 sticky top-0">
                      {SYSTEM_COLS.map((h, i) => (
                        <th key={i} className={`px-2 py-1.5 text-left font-medium border-b border-slate-200 whitespace-nowrap ${mapping[i] !== undefined ? 'text-slate-500' : 'text-slate-300 line-through'}`}>
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {mappedRows.slice(0, 10).map((row, ri) => (
                      <tr key={ri} className={ri % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                        {row.map((cell, ci) => (
                          <td key={ci} className={`px-2 py-1 border-b border-slate-100 truncate max-w-24 ${mapping[ci] === undefined ? 'text-slate-200' : 'text-slate-700'}`}>
              ...
              <div className="flex justify-between items-center mt-6">
                <button className="btn-secondary text-xs" onClick={() => { setImportStep('upload'); setMapping({}); }}>← Back</button>
                <div className="flex gap-2">
                  <button className="btn-secondary" onClick={() => { setImportOpen(false); setImportStep('upload'); setMapping({}); }}>Cancel</button>
