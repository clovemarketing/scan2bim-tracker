...
import {
  RefreshCw, MessageSquare, Clock, Users, Briefcase, TrendingUp, Filter, X, BarChart2,
  ClipboardList, Pencil, Search, Save, ChevronDown, Check,
} from 'lucide-react';
import {
...
export default function FeedbackPage({ toast }) {
  ...
  const [clientRowIndices, setClientRowIndices] = useState([]);
  const [internalRowIndices, setInternalRowIndices] = useState([]);
  const [clientTmMap, setClientTmMap] = useState({});
  const [internalTmMap, setInternalTmMap] = useState({});
  const [clientLogEntries, setClientLogEntries] = useState({});
  const [internalLogEntries, setInternalLogEntries] = useState({});
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [cRes, iRes, sett] = await Promise.all([
        api.feedbackProjects(),
        api.internalFeedbackProjects(),
        api.settings().catch(() => ({ projStatuses: [] })),
      ]);
      setClientRows(cRes.data || []);
      setInternalRows(iRes.data || []);
      setClientRowIndices(cRes.rowIndices || []);
      setInternalRowIndices(iRes.rowIndices || []);
      setClientTmMap(cRes.teamMembersMap || {});
      setInternalTmMap(iRes.teamMembersMap || {});
      setClientLogEntries(cRes.logEntries || {});
      setInternalLogEntries(iRes.logEntries || {});
  ...
  const rows = activeTab === 'client' ? clientRows : internalRows;
  const rowIndices = activeTab === 'client' ? clientRowIndices : internalRowIndices;
  const tmMap = activeTab === 'client' ? clientTmMap : internalTmMap;
  const logEntries = activeTab === 'client' ? clientLogEntries : internalLogEntries;
  const derivedSuffix = activeTab === 'client' ? 'FB-Client' : 'FB-Int';
  ...
  const leadOptions = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      if (r[22]) leads.add(r[22]);
      if (r[23]) leads.add(r[23]);
  ...
  const allLogEntries = useMemo(() => {
    const entries = [];
    filtered.forEach((r) => {
      const derivedId = `${r[0]}-${derivedSuffix}`;
      const le = logEntries[derivedId] || [];
      const projectLead = r[21] || r[22] || r[23] || r[4] || '';
      le.forEach((e) => {
        entries.push({
          ...e,
          projName: r[1] || r[0],
  ...
  }, [filtered, logEntries, derivedSuffix]);

  // ── Stats ──
  const stats = useMemo(() => {
    const totalFBHrsCol = activeTab === 'client' ? 19 : 17;
    const totalFBHrs = filtered.reduce((s, r) => s + (parseFloat(r[totalFBHrsCol]) || 0), 0);
    ...
    return {
      ...
      totalFBHrs, totalSpent, totalReqEff, totalActEff,
      sessions: allLogEntries.length,
      uniqueEmps,
      avgEff,
    };
  }, [filtered, allLogEntries, activeTab]);

  // ── Project Wise aggregation ──
  const projAnalytics = useMemo(() => {
    return filtered.map((r) => {
      const clientHrs = parseFloat(r[7]) || 0;
      const fbHrsCol = activeTab === 'client' ? 19 : 17;
      const fbHrs = parseFloat(r[fbHrsCol]) || 0;
      ...
      const projActualEff = fbHrs > 0 && spent > 0 ? (spent / fbHrs) : 0;
      const projReqEff = fbHrs > 0 && reqEff > 0 ? (reqEff / fbHrs) : 0;
  ...
  return (
    <div className="page">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="page-title flex items-center gap-2">
            <MessageSquare size={20} className={`text-${tabColor}-500`} />
            Feedback Analytics
          </h1>
          <p className="page-sub">{filtered.length} project{filtered.length !== 1 ? 's' : ''} · {stats.sessions} session{stats.sessions !== 1 ? 's' : ''} logged</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowFilters(!showFilters)} className={`btn-secondary gap-1.5 ${showFilters ? `border-${tabColor}-400 bg-${tabColor}-50` : ''}`}>
      ...
      {/* Feedback Type tab */}
      <div className="flex gap-1 bg-slate-100 p-0.5 rounded-xl mb-2 w-fit">
        <button
          onClick={() => { setActiveTab('client'); setFilterStatus('All'); setFilterLead('All'); }}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'client' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <MessageSquare size={14} /> Client Feedback
        </button>
        <button
          onClick={() => { setActiveTab('internal'); setFilterStatus('All'); setFilterLead('All'); }}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'internal' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <ClipboardList size={14} /> Internal Feedback
        </button>
      </div>

      {/* View mode tabs */}
      <div className={`flex gap-1 bg-${tabColor}-50/50 p-0.5 rounded-xl mb-5 w-fit`}>
        <button
          onClick={() => setViewMode('project')}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${viewMode === 'project' ? 'bg-white shadow text-slate-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Briefcase size={14} /> Project Wise
        </button>
        <button
          onClick={() => setViewMode('employee')}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${viewMode === 'employee' ? 'bg-white shadow text-slate-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Users size={14} /> Employee Wise
        </button>
        <button
          onClick={() => setViewMode('lead')}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${viewMode === 'lead' ? 'bg-white shadow text-slate-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <BarChart2 size={14} /> Team Lead Wise
        </button>
      </div>
       ...
       {showFilters && (
         <div className="card mb-5 flex flex-wrap gap-4 items-end">
           <div className="relative">
             ...
             <div className="relative">
               <button
                 ...
                 className="input w-64 text-xs text-left flex items-center justify-between gap-2"
               >
           ...
           <div>
             ...
             <select className="input w-40 text-xs" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
               {statuses.map((s) => <option key={s}>{s}</option>)}
             </select>
           </div>
           <div>
             <label className="label text-xs">Assigned Lead</label>
             <select className="input w-44 text-xs" value={filterLead} onChange={(e) => setFilterLead(e.target.value)}>
               {leadOptions.map((o) => <option key={o}>{o}</option>)}
             </select>
           </div>
           {(filterStatus !== 'All' || filterLead !== 'All' || selectedProjectIds.size > 0) && (
             <button className="btn-secondary text-xs self-end" onClick={() => { setFilterStatus('All'); setFilterLead('All'); setSelectedProjectIds(new Set()); setProjectFilterQuery(''); }}>
               <X size={12} /> Clear
             </button>
           )}
      ...
      {loading ? (
        ...
        <div className="card text-center py-16 text-slate-400">
          ...
          <p className="font-medium">No {activeTab === 'client' ? 'Client' : 'Internal'} Feedback session data yet</p>
          <p className="text-sm mt-1">Log hours against projects with "{activeTab === 'client' ? 'Client Feedback' : 'Internal Feedback'}" enabled.</p>
        </div>
      ) : viewMode === 'project' ? (
        <>
          {/* Project Wise Table */}
          <div className="card overflow-x-auto mb-6 p-0">
            <table className="min-w-full text-xs">
               <thead className="bg-slate-50 border-b border-slate-200">
                 <tr>
                   <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500">Project</th>
                   <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500">Client</th>
                   <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500">Status</th>
                   <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500">Lead</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Emps</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Sess</th>
                   <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Client Hrs</th>
                   <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">{activeTab === 'client' ? 'Client FB' : 'Int FB'} Hrs</th>
                   <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Spent</th>
                   <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Req Eff</th>
                   <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Act Eff</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Act Eff</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Req Eff</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Emp Avg</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500"></th>
                 </tr>
               </thead>
              <tbody>
                {projAnalytics.map((p, i) => (
                  <tr key={i} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-${tabColor}-50/30`}>
                    <td className="px-3 py-2.5">
                      <p className="font-medium text-slate-800 truncate max-w-[180px]" title={p.projName}>{p.projName}</p>
                      <p className="text-slate-400 font-normal">{p.projId}</p>
                    </td>
                    <td className="px-3 py-2.5 text-slate-600">{p.client}</td>
                    <td className="px-3 py-2.5"><Badge value={p.status} /></td>
                    <td className="px-3 py-2.5 text-slate-600">{p.projectLead}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{p.empCount}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{p.sessions}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-blue-600">{toHhmm(p.clientHrs * 60)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-amber-700">{toHhmm(p.fbHrs * 60)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-indigo-600 font-semibold">{toHhmm(p.spent)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(p.reqEff)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(p.actEff)}</td>
                     <td className="px-3 py-2.5 text-center"><EffBadge value={p.projActualEff} /></td>
                     <td className="px-3 py-2.5 text-center"><EffBadge value={p.projReqEff} /></td>
                     <td className="px-3 py-2.5 text-center"><EffBadge value={p.empAvgEff} /></td>
                ...
                <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold">
                  <tr>
                    <td className="px-3 py-2.5" colSpan={5}>Total ({projAnalytics.length} projects)</td>
                    <td className="px-3 py-2.5 text-center text-slate-700">{projAnalytics.reduce((s, p) => s + p.sessions, 0)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-blue-600">{toHhmm(projAnalytics.reduce((s, p) => s + p.clientHrs, 0) * 60)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-amber-700">{toHhmm(projAnalytics.reduce((s, p) => s + p.fbHrs, 0) * 60)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-indigo-600">{toHhmm(stats.totalSpent)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(stats.totalReqEff)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(stats.totalActEff)}</td>
                    <td className="px-3 py-2.5 text-center"><EffBadge value={stats.totalFBHrs > 0 && stats.totalSpent > 0 ? stats.totalSpent / stats.totalFBHrs : 0} /></td>
                    <td className="px-3 py-2.5 text-center"><EffBadge value={stats.totalFBHrs > 0 && stats.totalReqEff > 0 ? stats.totalReqEff / stats.totalFBHrs : 0} /></td>
                    <td className="px-3 py-2.5 text-center"><EffBadge value={stats.avgEff} /></td>
            ...
            <table className="min-w-full text-xs">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500">EMP ID</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Projects</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Sessions</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Spent Hrs</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Req Eff Hrs</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Act Eff Hrs</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Avg Efficiency</th>
                </tr>
              </thead>
              <tbody>
                {empAnalytics.map((e, i) => (
                  <tr key={i} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-${tabColor}-50/30`}>
                    <td className="px-3 py-2.5 font-medium text-slate-800">{e.empName}</td>
                    <td className="px-3 py-2.5 text-slate-400">{e.empId}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{e.projCount}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{e.sessions}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-indigo-600 font-semibold">{toHhmm(e.spent)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(e.reqEff)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(e.actEff)}</td>
                    <td className="px-3 py-2.5 text-center"><EffBadge value={e.avgEff} /></td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold">
                <tr>
                  <td className="px-3 py-2.5" colSpan={3}>Total ({empAnalytics.length} employees)</td>
                  <td className="px-3 py-2.5 text-center text-slate-700">{empAnalytics.reduce((s, e) => s + e.sessions, 0)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-indigo-600">{toHhmm(stats.totalSpent)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(stats.totalReqEff)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(stats.totalActEff)}</td>
                  <td className="px-3 py-2.5 text-center"><EffBadge value={stats.avgEff} /></td>
        ...
        <>
          ...
          <div className="card overflow-x-auto mb-6 p-0">
            <table className="min-w-full text-xs">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500">Team Lead</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Employees</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Projects</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Sessions</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Spent Hrs</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">Req Eff Hrs</th>
              ...
              <tbody>
                {leadAnalytics.map((l, i) => (
                  <tr key={i} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-${tabColor}-50/30`}>
                    <td className="px-3 py-2.5 font-medium text-slate-800">{l.leadName}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{l.empCount}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{l.projCount}</td>
                    <td className="px-3 py-2.5 text-center text-slate-500">{l.sessions}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-indigo-600 font-semibold">{toHhmm(l.spent)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(l.reqEff)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(l.actEff)}</td>
                    <td className="px-3 py-2.5 text-center"><EffBadge value={l.avgEff} /></td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold">
                <tr>
                  <td className="px-3 py-2.5">Total ({leadAnalytics.length} leads)</td>
                  <td className="px-3 py-2.5 text-center text-slate-700">{stats.uniqueEmps}</td>
                  <td className="px-3 py-2.5 text-center text-slate-700">{projAnalytics.length}</td>
                  <td className="px-3 py-2.5 text-center text-slate-700">{stats.sessions}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-indigo-600">{toHhmm(stats.totalSpent)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(stats.totalReqEff)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(stats.totalActEff)}</td>
                  <td className="px-3 py-2.5 text-center"><EffBadge value={stats.avgEff} /></td>
      ...
      <div className="mt-4 text-xs text-slate-400 flex flex-wrap gap-4">
        <span>Feedback Hrs from PROJECTS sheet</span>
        <span>Project Actual Efficiency = (Spent Hrs ÷ Feedback Hrs)</span>
        <span>Project Req Efficiency = (Req Eff Hrs ÷ Feedback Hrs)</span>
