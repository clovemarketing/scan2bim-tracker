...
export default function FeedbackPage({ toast }) {
  ...
  const leadOptions = useMemo(() => {
    const leads = new Set();
    const leadCol = activeTab === 'client' ? 23 : 22;
    rows.forEach((r) => {
      if (r[leadCol]) leads.add(r[leadCol]);
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      const lead = activeTab === 'client' ? (r[23] || r[4]) : (r[22] || r[4]);
  ...
  const allLogEntries = useMemo(() => {
    ...
    filtered.forEach((r) => {
      ...
      const projectLead = activeTab === 'client' ? (r[23] || r[4] || '') : (r[22] || r[4] || '');
      le.forEach((e) => {
        entries.push({
          ...e,
          projName: r[1] || r[0],
    ...
    <div className="page">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 self-start sm:self-auto flex-wrap">
          ...
          <button onClick={() => { Promise.all([api.syncFeedback(), api.syncInternalFeedback()]).then(() => toast.success('Feedback sheets synced')).catch((e) => toast.error(e.message)).then(load); }} className="btn-secondary gap-1 px-2 sm:px-3 text-[11px] sm:text-xs">
      ...
      {/* Feedback Type tab */}
      <div className="flex gap-1 bg-slate-100 p-0.5 rounded-xl mb-2 flex-wrap w-full sm:w-fit">
        <button
          onClick={() => { setActiveTab('client'); setFilterStatus('All'); setFilterLead('All'); }}
          className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all ${activeTab === 'client' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <MessageSquare size={13} className="sm:size-[14px]" /> Client
        </button>
        <button
          onClick={() => { setActiveTab('internal'); setFilterStatus('All'); setFilterLead('All'); }}
          className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all ${activeTab === 'internal' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <ClipboardList size={13} className="sm:size-[14px]" /> Internal
        </button>
      </div>
      ...
      <div className={`flex gap-1 bg-${tabColor}-50/50 p-0.5 rounded-xl mb-5 flex-wrap w-full sm:w-fit`}>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all ${viewMode === 'employee' ? 'bg-white shadow text-slate-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Users size={13} className="sm:size-[14px]" /> Employee
        </button>
        <button
          onClick={() => setViewMode('lead')}
          className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all ${viewMode === 'lead' ? 'bg-white shadow text-slate-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
            ...
            <table className="min-w-[750px] md:min-w-full text-xs">
               <thead className="bg-slate-50 border-b border-slate-200">
                 <tr>
                   ...
                   <th className="px-2 sm:px-3 py-2 text-right text-[10px] sm:text-xs font-semibold text-slate-500 hidden lg:table-cell">Client Hrs</th>
                   <th className="px-2 sm:px-3 py-2 text-right text-[10px] sm:text-xs font-semibold text-slate-500">{activeTab === 'client' ? 'FB' : 'Int'} Hrs</th>
                   ...
                   <th className="px-2 sm:px-3 py-2 text-center text-[10px] sm:text-xs font-semibold text-slate-500 hidden lg:table-cell">Actual Eff</th>
                   <th className="px-2 sm:px-3 py-2 text-center text-[10px] sm:text-xs font-semibold text-slate-500 hidden lg:table-cell">Req Eff</th>
                   <th className="px-2 sm:px-3 py-2 text-center text-[10px] sm:text-xs font-semibold text-slate-500 hidden lg:table-cell">Emp Avg</th>
              ...
              <tbody>
                {projAnalytics.map((p, i) => (
                  <tr key={i} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-${tabColor}-50/30`}>
                    ...
                    <td className="px-2 sm:px-3 py-2 text-right font-mono text-[10px] sm:text-sm text-blue-600 hidden lg:table-cell">{toHhmm(p.clientHrs * 60)}</td>
                    <td className="px-2 sm:px-3 py-2 text-right font-mono text-[10px] sm:text-sm text-amber-700 font-semibold">{toHhmm(p.fbHrs * 60)}</td>
                ...
                <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold">
                  <tr>
                    ...
                    <td className="px-2 sm:px-3 py-2 text-center hidden lg:table-cell"><EffBadge value={stats.totalFBHrs > 0 && stats.totalSpent > 0 ? stats.totalSpent / stats.totalFBHrs : 0} /></td>
                    <td className="px-2 sm:px-3 py-2 text-center hidden lg:table-cell"><EffBadge value={stats.totalFBHrs > 0 && stats.totalReqEff > 0 ? stats.totalReqEff / stats.totalFBHrs : 0} /></td>
