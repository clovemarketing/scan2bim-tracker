...
function MonthCalendar({ year, month, employees, rotOffs }) {
        ...
        <thead>
          <tr className="bg-slate-50">
            ...
            {Array.from({ length: numDays }, (_, i) => {
              ...
              return (
                <th key={i} className={`th text-xs text-center min-w-[32px] px-1 ${dow === 'Sun' ? 'bg-red-50' : ''}`}>
            ...
            return (
              <tr key={emp[0]} className="tr">
                ...
                {Array.from({ length: numDays }, (_, i) => {
                  ...
                  let bg = '';
                  if (isRotOff) { bg = 'bg-purple-100 text-purple-600 font-semibold'; }
                  else if (isWeekOff) { bg = 'bg-red-100 text-red-500'; }
                  else if (isSun) { bg = 'bg-slate-50 text-slate-400'; }
                  const label = isRotOff ? 'R' : isWeekOff ? 'W' : '✓';
                  return (
...
export default function ShiftRoster({ toast }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [empData, settData] = await Promise.all([api.employees(), api.settings()]);
  ...
  if (loading) return (
    <div className="page flex items-center justify-center h-full">
      <span className="text-slate-400 animate-pulse">Loading roster…</span>
    </div>
  );
      ...
      {view === 'roster' ? (
        <>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-purple-100 border border-purple-200" /> Rotational Off
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-red-100 border border-red-200" /> Week Off
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-slate-50 border border-slate-200" /> Sunday
              </span>
            </div>
          </div>
          ...
          {activeEmp.length === 0 ? (
            ...
            <MonthCalendar year={year} month={month} employees={employees} rotOffs={rotOffs} />
          )}
            ...
            <div className="flex items-center gap-2 mb-4 flex-wrap">
              ...
              <div className="flex gap-1 bg-slate-100 p-0.5 rounded-lg">
                {FILTER_OPTS.map((f) => {
                  ...
                  const isAlert = f.key !== 'all' && filteredMatrix.length > 0;
                  return (
        ...
        <Modal title={`Edit Shift & Week Off — ${editModal[1] || editModal[0]}`} onClose={() => setEditModal(null)}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="input" value={editForm.shift} onChange={(e) => setEditForm((f) => ({ ...f, shift: e.target.value }))}>
            ...
            <div>
              ...
              <select className="input" value={editForm.weekOff} onChange={(e) => setEditForm((f) => ({ ...f, weekOff: e.target.value }))}>
                <option value="">— Not Assigned —</option>
                {(settings?.weekoffs || []).map((w) => (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select className="input" value={editForm.rotDay1} onChange={(e) => setEditForm((f) => ({ ...f, rotDay1: e.target.value }))}>
                  ...
                  <div>
                    ...
                    <select className="input" value={editForm.rotDay2} onChange={(e) => setEditForm((f) => ({ ...f, rotDay2: e.target.value }))}>
                      <option value="">— Auto —</option>
                      {Array.from({ length: numDays }, (_, i) => (
                        <option key={i + 1} value={i + 1}>Day {i + 1}</option>
                      ))}
      ...
      {bulkModal && (
        <Modal title={`Assign Shift & Week Off (${selected.size} employees)`} onClose={() => setBulkModal(false)}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="label">Week Off</label>
              <select className="input" value={bulkForm.weekOff} onChange={(e) => setBulkForm((f) => ({ ...f, weekOff: e.target.value }))}>
                <option value="">— Keep unchanged —</option>
                {(settings?.weekoffs || []).map((w) => (
                  <option key={w.name} value={w.name}>{w.name}</option>
                ))}
