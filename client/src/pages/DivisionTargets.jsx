...
export default function DivisionTargets({ toast, currentUser }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [p, e, ph] = await Promise.all([api.projects(), api.employees(), api.projectHours()]);
      setProjects(p.data || []);
      setEmployees(e.data || []);
    ...
    <div className="page">
      ...
      <div className="flex gap-1 bg-slate-100 p-0.5 rounded-xl mb-5 w-fit">
        <button
          ...
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'targets' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
        >
          ...
          <div className="card mb-5">
            ...
            <div className="flex items-center gap-4 mb-4 p-3 bg-indigo-50 rounded-lg">
              <div>
                ...
                <input type="number" min="0" step="10" className="input w-36 text-sm font-semibold mt-1" value={divisionTarget} onChange={(e) => setDivisionTarget(e.target.value)} readOnly={readOnly || !editing} disabled={readOnly || !editing} />
              </div>
