    ...
    <div className="page">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="page-sub">{filtered.length} project{filtered.length !== 1 ? 's' : ''} with QA/QC flag · {stats.sessions} session{stats.sessions !== 1 ? 's' : ''} logged</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowFilters(!showFilters)} className={`btn-secondary gap-1.5 ${showFilters ? 'border-amber-400 bg-amber-50' : ''}`}>
      ...
      {/* View mode tabs */}
      <div className="flex gap-1 bg-amber-50/50 p-0.5 rounded-xl mb-5 w-fit">
        <button
          onClick={() => setViewMode('project')}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${viewMode === 'project' ? 'bg-white shadow text-amber-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Briefcase size={14} /> Project Wise
        </button>
        <button
          onClick={() => setViewMode('employee')}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${viewMode === 'employee' ? 'bg-white shadow text-amber-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Users size={14} /> Employee Wise
        </button>
        <button
          onClick={() => setViewMode('lead')}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${viewMode === 'lead' ? 'bg-white shadow text-amber-700' : 'text-slate-500 hover:text-slate-700'}`}
        >
            ...
            <table className="min-w-full text-xs">
               <thead className="bg-slate-50 border-b border-slate-200">
                 <tr>
                   ...
                   <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500">QC Hrs</th>
                   ...
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">QC Actual Eff</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">QC Req Eff</th>
                   <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500">Emp Avg Eff</th>
              ...
              <tbody>
                {projAnalytics.map((p, i) => (
                  <tr key={i} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-amber-50/30`}>
                    ...
                    <td className="px-3 py-2.5 text-right font-mono text-amber-700 font-semibold">{toHhmm(p.qcHrs * 60)}</td>
                ...
                <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold">
                  <tr>
                    ...
                    <td className="px-3 py-2.5 text-right font-mono text-amber-700">{toHhmm(projAnalytics.reduce((s, p) => s + p.qcHrs, 0) * 60)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-indigo-600">{toHhmm(stats.totalSpent)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-slate-700">{toHhmm(stats.totalReqEff)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-cyan-600">{toHhmm(stats.totalActEff)}</td>
                    <td className="px-3 py-2.5 text-center">
                      <EffBadge value={
                        stats.totalSpent > 0 
              ...
              <tbody>
                {leadAnalytics.map((l, i) => (
                  <tr key={i} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-amber-50/30`}>
                    ...
                    <td className="px-3 py-2.5 text-right font-mono text-amber-700 font-semibold">{toHhmm(l.totalQcHrs * 60)}</td>
              ...
              <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold">
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-center text-slate-700">{stats.sessions}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-amber-700">{toHhmm(stats.totalQCHrs * 60)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-indigo-600">{toHhmm(stats.totalSpent)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-violet-600">{toHhmm(stats.totalReqEff)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-emerald-600">{toHhmm(stats.totalActEff)}</td>
                  <td className="px-3 py-2.5 text-center"><EffBadge value={stats.avgEff} /></td>
