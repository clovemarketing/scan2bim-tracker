...
const SHEETS = [
  ...
  { key: 'DIV_TARGETS', label: 'Division Targets', desc: 'Team lead monthly target hours' },
  { key: 'HOLIDAYS', label: 'Holidays', desc: 'Annual holiday calendar' },
];
...
export default function DataManagement({ toast }) {
  ...
  return (
    <div className="page">
      ...
      <p className="page-sub !mb-0">Export backup · manage Google Sheets data</p>
      ...
      {/* Sheets Overview */}
      <section className="card mb-6">          <h2 className="text-base font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <Database size={18} className="text-slate-400 shrink-0" /> Google Sheets
          </h2>
          ...
          <div className="divide-y divide-slate-50">
            {SHEETS.map(({ key, label, desc }) => (
              <div key={key} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 py-2.5 sm:py-3">
                ...
                <button
                  ...
                  className="btn-secondary text-xs text-red-600 border-red-100 hover:bg-red-50 gap-1.5 py-1.5 self-start sm:self-center"
                >
                  {clearing === key ? (
