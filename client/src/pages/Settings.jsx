    ...
    <div className="page">
      ...
      {activeGroup === 'users' ? (
        ...
        <div className="card">
            ...
            <div className="flex items-center gap-2">
              {editingTab === activeTab ? (
                <>
                  ...
                  <button onClick={() => { load(); setEditingTab(null); }} className="btn-secondary py-1.5 px-3 text-xs">
                    Cancel
                  </button>
                  <button onClick={saveCategory} disabled={saving} className="btn-primary py-1.5 px-3 text-xs">
                    <Save size={13} /> {saving ? 'Saving…' : 'Save'}
                  </button>
                </>
              ) : (
                <button onClick={() => setEditingTab(activeTab)} className="btn-secondary py-1.5 px-3 text-xs">
                  <Pencil size={13} /> Edit
                </button>
              )}
            </div>
          ...
          {(function() {
            if (loading) {
              return <p className="text-slate-400 animate-pulse py-8 text-center">Loading…</p>;
            }
            if (tabRows.length === 0) {
              return (
            ...
            if (editingTab === activeTab) {
                ...
                return (
                  <div className="rounded-xl border border-slate-100 overflow-hidden">
                    ...
                    {tabIndices.map((globalIdx, ti) => {
                      ...
                      return (
                        <div key={ti} className="px-4 py-2 border-b border-slate-50 last:border-0 hover:bg-slate-50/60 transition-colors"
              ...
              return (
                <div className="rounded-xl border border-slate-100 overflow-hidden">
                  <div className="bg-slate-50 px-4 py-2.5 border-b border-slate-100"
                    style={{ display: 'grid', gridTemplateColumns: colGrid, gap: '0.75rem' }}>
                    {hasValueCol ? (
                      <>
                  ...
                  {tabIndices.map((globalIdx, ti) => {
                    ...
                    return (
                      <div key={ti}
                        className={`px-4 py-2 border-b border-slate-50 last:border-0 hover:bg-slate-50/60 transition-colors`}
                        style={{ display: 'grid', gridTemplateColumns: colGrid, gap: '0.75rem', alignItems: 'center' }}>
            ...
            if (activeTab === 'PROD_MATRIX') {
              return (
                <div className="rounded-xl border border-slate-100 overflow-hidden">
                  <div className="bg-slate-50 px-4 py-2.5 border-b border-slate-100"
                    style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: '0.75rem' }}>
                  ...
                  {tabIndices.map((globalIdx, ti) => {
                    ...
                    const maxVal = parseFloat((row[3] || '').replace('Max:', '').trim()) || 0;
                    return (
                      <div key={ti} className="px-4 py-2 border-b border-slate-50 last:border-0"
                        style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: '0.75rem', alignItems: 'center' }}>
            ...
            return (
              <div className="rounded-xl border border-slate-100 overflow-hidden">
                <div className="bg-slate-50 px-4 py-2.5 border-b border-slate-100"
                  style={{ display: 'grid', gridTemplateColumns: hasValueCol ? '1.5fr 1.5fr 1.5fr' : '1.5fr 2fr', gap: '0.75rem' }}>
                  {hasValueCol ? (
                    <>
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{colHeaders[0]}</span>
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{colHeaders[1]}</span>
                ...
                {tabIndices.map((globalIdx, ti) => {
                  ...
                  return (
                    <div key={ti}
                      className="px-4 py-2 border-b border-slate-50 last:border-0"
                      style={{ display: 'grid', gridTemplateColumns: hasValueCol ? '1.5fr 1.5fr 1.5fr' : '1.5fr 2fr', gap: '0.75rem', alignItems: 'center' }}>
