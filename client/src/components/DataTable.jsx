...
const PAGE_OPTS = [10, 20, 50, 100, Infinity];
...
export default function DataTable({
  ...
  mobileCard = null, // (row, origIdx, isSelected, sheetRow) => ReactNode — renders mobile card view
}) {
  ...
  // Debounce search input — filter runs 300 ms after typing stops
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* ── Table ── */}
      <div className={`overflow-x-auto rounded-xl border border-slate-100 shadow-sm ${mobileCard ? 'hidden md:block' : ''}`}>
      ...
      {mobileCard && (
        <div className="md:hidden space-y-2">
          {paginated.length === 0 ? (
            ...
            paginated.map(([origIdx, row]) => {
              const isSelected = selected.has(origIdx);
              const sheetRow = rowIndices ? rowIndices[origIdx] : origIdx + 2;
              return (
                <div key={origIdx}
                  onClick={() => selectable && toggleRow(origIdx)}
                  className={`rounded-xl border bg-white px-3 py-2.5 transition-colors cursor-pointer ${isSelected ? 'border-indigo-300 ring-1 ring-indigo-200' : 'border-slate-100'}`}>
                  {selectable && (
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-sm text-slate-500">
        <div className="flex items-center gap-2 flex-wrap justify-center">
          ...
          <select className="text-xs border border-slate-200 rounded-md px-2 py-1 bg-white"
            value={pageSize} onChange={(e) => setPageSize(e.target.value === 'Infinity' ? Infinity : Number(e.target.value))}>
            {PAGE_OPTS.map((n) => <option key={n} value={n}>{n === Infinity ? 'All' : n}</option>)}
          </select>
        </div>
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
